...
class SocialTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: FirebaseFirestore.instance.collection(FirebaseConstants.discussionDb).doc(discussionId).snapshots(),
                  ...
                  children: [
                    ...
                    BlocBuilder<LikeBloc, LikeState>(
                      builder: (context, state) {
                        if (state is AddLikeState ||snapshot.data!['likes'].contains(UserDbFunctions().userId)) {
                          return TextButton.icon(
                              onPressed: () {
                                BlocProvider.of<LikeBloc>(context).add(
                                    RemoveLikeEvent(discussionId: discussionId));
                              },
                              icon: const Icon(
                                Icons.thumb_up,
                          ...
                          return TextButton.icon(
                              onPressed: () {
                                BlocProvider.of<LikeBloc>(context).add(
                                    AddLikeEvent(discussionId: discussionId));
                              },
                              icon: const Icon(
                                Icons.thumb_up,
                    ...
                    TextButton.icon(
                        onPressed: () {
                          Navigator.of(context).push(MaterialPageRoute(builder: (ctx)=>ContributeToDiscussionPage(title: title, discussionId: discussionId,count:discussions)));
                        },
                        icon: const Icon(
